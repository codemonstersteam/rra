...
### Часть 1. Структурная проверка (статика)

- **Лестница вглубь**: README → component-tests/ → docs/architecture.md → docs/adr/ → CONTRIBUTING.md. Отсутствие звена — снижение, не блокировка.
- **Pipe-формат API**: эндпоинты описаны как пошаговый поток ввод → процесс → выход. Если шагов > 5 — флаг «API делает слишком много».
- **Дублирование**: повторяющиеся фрагменты > N токенов в разных файлах — нарушение «единого источника правды».
- **Drift**: `git log` по `docs/` vs по коду. Если средний возраст docs существенно старше последних правок кода в соответствующем slice — флаг.
- **Явные запреты**: ищем формулировки «НЕ делай», «запрещено» — индикатор зрелой документации для агентов.

### Часть 2. JTBD-симуляция через LLM

Четыре независимых прогона, по одному на каждого потребителя. У каждого свой job, бюджет и критерий успеха:
...
### Часть 3. Согласованность доки и кода
...
Четыре независимых score (по одному на JTBD) + список пробелов. **Не усредняем**: «документация для всех = ни для кого», поэтому и оценка не одна цифра.
